...
AIManager.setProperTarget = function(group) {
    ...
    if (line.match(/FIRST/i)) {
      action.setTarget(0);
        ...
        if (Game_BattlerBase.prototype.hasOwnProperty(param)) {
            return this.setMaxStatusTarget(group, param);
        } else {
            //alert('error');
      ...
      */
        ...
        var param = String(RegExp.$1);
        if (Game_BattlerBase.prototype.hasOwnProperty(param)) {
            return this.setMinStatusTarget(group, param);
        } else {
            //alert('error');
            return action.setTarget(randomTarget.index())
        }
...
AIManager.setMaxStatusTarget = function(group, status) {
    var maintarget = group[0];
    for (var i = 0; i < group.length; ++i) {
        var target = group[i];
        if (target[status] > maintarget[status]) {
            maintarget = target;
        }
    }
...
AIManager.setMinStatusTarget = function(group, status) {
    var maintarget = group[0];
    for (var i = 0; i < group.length; ++i) {
        var target = group[i];
        if (target[status] < maintarget[status]) {
